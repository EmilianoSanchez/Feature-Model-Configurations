...
public class Configuration implements Cloneable, Serializable {
	...
	public Configuration(FeatureModel model,FeatureState state) {
		this(model.getNumFeatures(),state);
		this.model = model;
	}
	...
	public Configuration(int numOfFeatures,FeatureState state) {
		this.states = new FeatureState[numOfFeatures];
	...
	public List<Integer> getSelectedChildren(int featureId){
		
		int[] children = this.model.getChildren(featureId);
		List<Integer> result=new ArrayList<>(children.length);
		for(int child:children) {
			if(this.states[child]==FeatureState.SELECTED) {
				result.add(child);
			}
		}
		return result;
